# A friend of yours wants to build a basic search engine and starts
# reading up on information extraction. He decides to try a simple
# implementation of TF-IDF, a measure of the information content a term
# holds for a particular document in a document collection. However, he
# just started learning to code and somehow his numbers do not match
# what is expected. He knows you're enrolled in an awesome developer
# school, so he asks you to have a look.

# Term frequency - inverse document frequency: A measure of how
# important a term is to a document in a collection of documents (the
# importance increases proportionally to the term frequency in the
# document, but decreases with the frequency of the word across
# documents)

def tfidf(term, document, documents)
  tfidf = tf(term, document) * idf(term, documents).round(1)
  format("%0.1f", tfidf)
end
...
def idf(term, documents)
  ...
  Math.log(number_of_documents_with_term.to_f / number_of_documents)
end
