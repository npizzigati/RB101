...
# A stack is a list of values that grows and
# shrinks dynamically. In ruby, a stack is easily implemented as an Array that
# just uses the #push and #pop methods.
...
# minilang('3 PUSH 4 PUSH 5 PUSH PRINT ADD PRINT POP PRINT ADD PRINT') # 5 # 10
# # 4 # 7
...
Output: the result of executing commands
Rules:
Explicit: See above; no unexpected tokens
Implicit: NA
Algorithm:
  initialize register to zero and stack to empty array
  split instructions string and iterate through it
  if instruction is a string representation of an integer:
    set the register equal to it
  if instruction is PUSH
    push number in register onto stack, also leaving it in register
  if instruction is POP
    pop number off stack and set register equal to it
  if instruction is MULT
    pop number of stack, multiply it by number in register,
    replace number in register with product
  if instruction is DIV
    pop number of stack, integer divide number in register by it,
    replace number in register with quotient
  if instruction is MOD, ADD, SUB,
    pop number of stack and do the appropriate operation.
...
def minilang(instructions)
  ...
  instructions.split.each do |instruction|
    if represents_integer?(instruction)
...
def represents_integer?(string_value)
  string_value == string_value.to_i.to_s
end

# Further exploration: Add some error handling to your method. In
# particular, have the method detect empty stack conditions, and
# invalid tokens in the program, and report those. Ideally, the method
# should return an error message if an error occurs, and nil if the
