...
rules:
-----
explicit: - output array is input array sorted in place
implicit: - array should be sorted in ascending order
          - array may consist of numbers or strings but not both

data structures and algorithm
*****************************
Use a nested loop. Outer loop: size of array (no index necessary). Inner loop:
size of array - 1, starting at one. In inner loop, compare n against n - 1 and
switch their positions if n - 1 is greater than n. Increment changes
counter. Break out of outer loop if change counter is zero.
...
def bubble_sort(arr)
  ...
  last_swap_pos = 0
  size.times do
    1.upto(swap_stop) do |n|
      if arr[n - 1] > arr[n]
        arr[n - 1], arr[n] = arr[n], arr[n - 1]
        last_swap_pos = n
      end
    end
    break if last_swap_pos == 0

    swap_stop = last_swap_pos
    last_swap_pos = 0
  end
  arr
end
